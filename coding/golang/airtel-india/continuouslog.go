...
func main() {
	...
	pidfile, _ := os.Create("tahseen.pid")
	...
	signal.Notify(channelValue, syscall.SIGHUP)

	var signalValue os.Signal

	go func() {
		for {
			signalValue = <-channelValue
		}
	}()
	...
	for line := range infile.Lines {
		...
		if len(transaction_id) == 36 {
			...
			if err == nil {
				...
				outfile.WriteString(datevalue + "," + timevalue + "," + transaction_id + "," + strconv.Itoa(adcode) + "," + caller + "," + called + "," + handset + "," + htype + "," + circle + "," + hub + "," + duration + "," + dtmf + "\n")
